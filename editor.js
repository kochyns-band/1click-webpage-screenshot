...
//		c.fillStyle=currentLevel.color;
    ...
    function createCanvas() {
        ...
        $canvas.add(newImg).css({
            position: 'absolute',
            'z-index': czindex++,
            left: 0,
            'co': $canvasData.position().left,
            'top': 0,
            'color': $canvasData.offset().top - scrollTop
        });
    }
...
//		$('#divCanvasData').css({'width':x2-x1,height:y2-y1})
    ...
    function pos(e) {
        ans = {
            x: clip.rx1 + e.pageX - $('#divCanvasData').offset().left + $('#divCanvasData').scrollLeft(),
            y: clip.ry1 + e.pageY - $('#divCanvasData').offset().top + $('#divCanvasData').scrollTop()
        };
...
//		id=inX.ctx.createImageData(inX.canvas.width,inX.canvas.height);
    ...
    tool.spray.up = function () {
        tool.status = 'ready';
    };
////    Free
    tool.free.up = function () {
        tool.status = 'ready';
    };
...
//		tool[currentLevel.type].draw(inX.canvas,inX.ctx,currentLevel)
    ...
    function tryFunc(inFunc) {
        if ($.isFunction(inFunc)) inFunc();
    }
    ...
    function onResize() {
        if (true) {
            ...
            $('#clipper').css({
                'height': clip.ry2 - clip.ry1,
                'overflow': 'hidden',
                'width': clip.rx2 - clip.rx1,
                'position': 'absolute'
            })
            //
            if (firstImage) {
        ...
        (function () {
            ...
            hr = $.ajax({
                ...
                complete:
                    function (a, b, c) {
                        if (cancel) {
                            $('#topText').html('Canceled!');
                            $('#save').add('#toGoogleDrive').add('#print').attr('disabled', null);
                            return;
                        }
                        noMoreWait();
...
//**********Drawing///
    ...
    function upload(inService) {
        ...
        if (imageChanged != localStorage['pngjpg'])
            window.setTimeout(function () {
                editor.createLastCanvas('share', inService)
            }, 0)
        else
            window.setTimeout(function () {
                shareService(inService, onlineUrl)
            }, 0)
        noMoreWait();
    }
    ...
    function toGoogleDrive() {
        ...
        pleaseWait();
        window.setTimeout(function () {
            editor.createLastCanvas('toGoogleDrive')
        }, 0)
    }
...
$(function () {
    ...
    $('.savetoodoo').on('click', function (e) {
        // debugger;
        // e.stopPropagation();
        var x = staticPlugin.getPluginByKey('savetoodoo');
        editor.createLastCanvas('toolbar', function (data) {
            x.run(data, e)
        })
