...
var api = {
    ...
    createTask: function () {
        console.log('createTask background');

        chrome.tabs.getSelected(null, function (tab) {
            var odoo_url = new URL(tab.url);
            chrome.storage.local.get('odoo_screenshots', function (data) {
                if ('odoo_screenshots' in data) {
                    data = data['odoo_screenshots'];
                    // create new task
                    $.ajax({
                        url: odoo_url.origin + '/smart_screenshots/tasks/',
                        type: 'post',
                        ...
                        if (response['result'] && response['result']['status'] === 'ok') {
                            chrome.tabs.query({active: true, currentWindow: true}, function (tabs) {
                                chrome.tabs.executeScript(
                                    tabs[0].id,
                                    {code: 'document.getElementsByClassName("smart_screenshot_button")[0].click();'});
                            });
                            chrome.storage.local.remove('odoo_screenshots');
                        }
                    })
                }
            });
    ...
    updateTask: function (screenshotType) {
        console.log('updateTaskOdoo background');
        chrome.tabs.getSelected(null, function (tab) {
            var odoo_url = new URL(tab.url);
            ...
            if (is_open_form_task_view) {
                var task_id = Number(odoo_url_params.get('#id'));
                chrome.storage.local.get('odoo_screenshots', function (data) {
                    if ('odoo_screenshots' in data) {
                        data = data['odoo_screenshots'];
                        if (screenshotType === 'solution') {
                            Object.keys(data).forEach(function (id) {
                                data[id]['type'] = 'solution';
                            })
                        }
                        ...
                        $.ajax({
                            ...
                            console.log(response);
                            console.log('smart_screenshot_button 111!!!');
                            if (response['result'] && response['result']['status'] === 'ok') {
                                chrome.tabs.query({active: true, currentWindow: true}, function (tabs) {
                                    chrome.tabs.executeScript(
                                        tabs[0].id,
        ...
        chrome.runtime.onMessage.addListener(function (data, sender, callback) {
            ...
            switch (data.data) {
                ...
                case "createTask":
                    api.createTask();
                    break;
                case "updateTaskProblem":
                    api.updateTask('problem');
                    break;
                case "updateTaskSolution":
                    api.updateTask('solution');
                    break;
                case "storageSet":
